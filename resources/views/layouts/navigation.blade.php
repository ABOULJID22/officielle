...
<header id="app-header" class="fixed top-0 z-50 w-full  backdrop-blur-lg bg-gray-900/30 shadow-sm transition-all duration-300 dark:bg-gray-900/80">
    <div class="mx-auto max-w-7xl px-6">
        <div class="flex h-20 items-center justify-between">
            ...
            <!-- Logo -->
            <a href="{{ url('/') }}{{ url('/') }}" class="flex items-center gap-3">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="hidden md:flex items-center gap-3">
                    @if (Route::has('login'))
                        @auth
                            <a href="{{ url('/') }}{{ url('/dashboard') }}" class="rounded-lg bg-[#4f6ba3] dark:bg-[#4f6ba3] dark:text-white px-5 py-2.5 text-sm font-semibold text-white shadow-md transition hover:bg-[#425a8a]">
                                Dashboard
                            </a>
                            <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button class="rounded-lg border border-red-500 px-4 py-2 text-sm font-medium text-red-500 hover:bg-red-500 hover:text-white transition">
                            Déconnexion
                        </button>
                    </form>
                        @else
                            <a href="{{ url('/') }}{{ route('login') }}" class="rounded-lg border border-[#4f6ba3] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#4f6ba3] hover:text-white dark:text-gray-200 dark:border-gray-200">
                                Connexion
                            </a>
                            @if (Route::has('register'))
                                <a href="{{ url('/') }}{{ route('register') }}" class="rounded-lg bg-[#4f6ba3] px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-gray-700 dark:bg-blue-600 dark:hover:bg-blue-700">
                                    Inscription
                                </a>
                            @endif
                        @endauth
              ...
              9.003 0 008.354-5.646z"/>
                  ...
                  <div id="mobile-menu" class="pointer-events-none absolute left-0 w-full origin-top scale-95 transform opacity-0 transition duration-200 ease-out md:hidden">
                      <div class="grid grid-cols-1 gap-y-2 rounded-xl bg-white p-4 shadow-xl ring-1 ring-gray-900/5 dark:bg-gray-800">
                          <a href="{{ url('/') }}#accueil" class="block rounded-lg px-4 py-2 text-base font-medium text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:text-[#4f6ba3]">Accueil</a>
                          ...
                          <a href="{{ url('/') }}#services" class="block rounded-lg px-4 py-2 text-base font-medium text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:text-[#4f6ba3]">Services</a>
                          ...
                          <div class="mt-4 border-t pt-4 space-y-2 border-[#4f6ba3] text-white dark:border-gray-600">
                              @if (Route::has('login'))
                                  @auth
                                      <a href="{{ url('/') }}{{ url('/dashboard') }}" class="block w-full rounded-lg bg-[#4f6ba3]  px-5 py-2.5 text-center text-sm font-semibold text-white shadow-md transition hover:bg-[#425a8a]">Dashboard</a>
                                  @else
                                      <a href="{{ url('/') }}{{ route('login') }}" class="block w-full rounded-lg bg-gray-100 px-5 py-2.5 text-center text-sm font-semibold text-gray-800 transition hover:bg-gray-200 dark:bg-gray-700 dark:text-white dark:hover:bg-gray-600">Connexion</a>
                                      @if (Route::has('register'))
                                          <a href="{{ url('/') }}{{ route('register') }}" class="block w-full rounded-lg bg-gray-800 px-5 py-2.5 text-center text-sm font-semibold text-white shadow-md transition hover:bg-gray-700 dark:bg-blue-600 dark:hover:bg-blue-700">Inscription</a>
                                      @endif
                                  @endauth
                              @endif
