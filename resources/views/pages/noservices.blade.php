...
<div class="bg-gray-50 font-sans text-[#283043] dark:bg-gray-900 dark:text-gray-100">
    ...
    <!-- FEATURE GRID -->
    <section class=" bg-gray-50/50 dark:bg-gray-900/50 relative">
        <div class="container mx-auto px-4 relative z-10">
            @php
                // language switcher: persist choice in session via ?lang=en or ?lang=fr
                if (request()->has('lang')) {
                ...
                $locale = session('site_locale', app()->getLocale());
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
                @php
                    // Load noservices from DB; fall back to empty collection
                    $dbCards = \App\Models\Noservice::orderBy('id', 'asc')->get();
                @endphp

                @forelse($dbCards as $card)
                    @php
                        // Normalise model attributes using selected locale
                        if ($locale === 'en') {
                            $title = $card->title ?? $card->titre ?? '';
                            $lead = $card->subtitle ?? $card->soustitre ?? '';
                            $items = $card->details ?? $card->detalserivces ?? [];
                            $result = $card->result ?? $card->resultats ?? '';
                        } else {
                            $title = $card->titre ?? $card->title ?? '';
                            $lead = $card->soustitre ?? $card->subtitle ?? '';
                            $items = $card->detalserivces ?? $card->details ?? [];
                            $result = $card->resultats ?? $card->result ?? '';
                        }
                        // If details stored as array of objects from Filament repeater, map to strings
                        $items = collect($items)->map(function ($it) {
                            if (is_array($it) && array_key_exists('text', $it)) {
                                return $it['text'];
                            }
                            return is_string($it) ? $it : (is_array($it) ? implode(' ', $it) : (string) $it);
                        })->filter()->values()->all();
                        
                        $accents = ['bg-blue-500', 'bg-indigo-500', 'bg-slate-700', 'bg-sky-500'];
                        $accentColor = $accents[($loop->index) % 4];
                    @endphp

                    <article x-data="{ open: false, isTouch: false }" 
                             @touchstart.passive="isTouch = true"
                             @mouseenter="if (!isTouch) open = true" 
                             @mouseleave="if (!isTouch) open = false"
                             @click="open = !open"
                             @click.outside="open = false"
                             class="group relative bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 dark:border-gray-700/50 flex flex-col h-full overflow-hidden isolate hover:-translate-y-1">
                        
                        <!-- Top Gradient Line -->
                        <div class="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-[#4f6ba3] to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>

                        <!-- Hover decorative blob -->
                        <div class="absolute -right-20 -top-20 w-48 h-48 rounded-full {{ $accentColor }} opacity-0 group-hover:opacity-5 transition-opacity duration-500 blur-3xl pointer-events-none"></div>

                        <div class="mb-5 relative z-10">
                            <h3 class="text-lg md:text-xl font-bold text-[#283043] dark:text-white mb-2 group-hover:text-[#4f6ba3] transition-colors duration-300">
                                {{ $title }}
                            </h3>
                            @if($lead)
                                <p class="text-gray-600 dark:text-gray-400 text-sm leading-relaxed">{{ $lead }}</p>
                            @endif
                        </div>

                        @if(!empty($items))
                            <ul class="space-y-3 mb-16 flex-grow relative z-10 transition-all duration-300" :class="{'mb-6': !open, 'mb-16': !open && false}">
                                @foreach($items as $item)
                                    <li class="flex items-start gap-3 group/item">
                                        <div class="mt-0.5 flex-shrink-0 w-5 h-5 rounded-full bg-blue-50 dark:bg-white/5 flex items-center justify-center group-hover/item:bg-[#4f6ba3] transition-colors duration-300 shadow-sm">
                                            <svg class="w-3 h-3 text-[#4f6ba3] group-hover/item:text-white transition-colors duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg>
                                        </div>
                                        <span class="text-gray-600 dark:text-gray-300 text-sm leading-snug">{{ $item }}</span>
                                    </li>
                                @endforeach
                            </ul>
                        @endif
                        ...
                        @if($result)
                            <div :class="open ? 'translate-y-0' : 'translate-y-full'" class="absolute bottom-0 left-0 right-0 bg-white/95 dark:bg-gray-800/95 backdrop-blur-md p-6 border-t border-gray-100 dark:border-gray-700 transform transition-transform duration-300 ease-out z-20 shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)]">
                                <div class="flex items-center gap-3 mb-3 text-[#4f6ba3]">
                                    ...
                                    <h4 class="font-bold uppercase tracking-wider text-[10px]">{{ __('services.card.result') }}</h4>
                                </div>
                                <p class="text-gray-800 dark:text-gray-200 leading-relaxed font-medium text-xs">
                                    <span class="whitespace-pre-wrap break-words block">{!! nl2br(e($result)) !!}</span>
                                </p>
                            </div>
                            
                            <!-- Hint text visible when not hovered -->
                            <div class="absolute bottom-6 left-6 right-6 transition-opacity duration-300 delay-75" :class="{ 'opacity-0': open }">
                ...
                @empty
                    <div class="col-span-full py-20 flex flex-col items-center justify-center text-center">
                        ...
                        <p class="text-gray-500 dark:text-gray-400 font-medium">{{ __('services.card.empty') }}</p>
                    </div>
                @endforelse
            </div>
        </div>
    ...
    <!-- PROMISE & CTA (responsive two-column: CTA left, Promise right) -->
    <section class="py-12">
        <div class="container mx-auto px-4">
            <!-- Updated to lg:grid-cols-2 for better tablet responsiveness -->
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">


                <!-- Promise: right column -->
                <div  id="les-leviers"  class="order-1">
                    <h3 class="text-xl md:text-2xl font-bold mb-6 text-[#283043] dark:text-gray-100 pl-4 border-l-4 border-[#4f6ba3]">{{ __('services.promise.title') }}</h3>
                    <!-- Updated inner spacing -->
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
                        <div class="text-center p-6 bg-white/5 rounded-xl border border-white/10 dark:border-gray-700 transform transition hover:-translate-y-1">
                            <div class="text-[#6e94c3] text-3xl mb-3 font-bold">01</div>
                            <h4 class="text-base font-semibold mb-2">{{ __('services.promise.1.title') }}</h4>
                            <p class="text-gray-500 dark:text-gray-300 text-xs">{{ __('services.promise.1.description') }}</p>
                        </div>

                        <div class="text-center p-6 bg-white/5 rounded-xl border border-white/10 dark:border-gray-700 transform transition hover:-translate-y-1">
                            <div class="text-[#6e94c3] text-3xl mb-3 font-bold">02</div>
                            <h4 class="text-base font-semibold mb-2">{{ __('services.promise.2.title') }}</h4>
                            <p class="text-gray-500 dark:text-gray-300 text-xs">{{ __('services.promise.2.description') }}</p>
                        </div>

                        <div class="text-center p-6 bg-white/5 rounded-xl border border-white/10 dark:border-gray-700 transform transition hover:-translate-y-1">
                            <div class="text-[#6e94c3] text-3xl mb-3 font-bold">03</div>
                            <h4 class="text-base font-semibold mb-2">{{ __('services.promise.3.title') }}</h4>
                            <p class="text-gray-500 dark:text-gray-300 text-xs">{{ __('services.promise.3.description') }}</p>
                        </div>
                    </div>
                </div>
                                ...
                                <!-- CTA: left column -->
                <div class="order-2">
                    <div class="bg-white dark:bg-gray-800 rounded-3xl p-4 sm:p-12 border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-lg transition-transform transform motion-safe:will-change-transform">
                        <h4 class="text-xl md:text-2xl lg:text-3xl font-bold text-[#283043] dark:text-gray-100 mb-2">{{ __('services.cta.title') }}</h4>
                        <p class="text-gray-600 dark:text-gray-300 text-base mb-6">{{ __('services.cta.description') }}</p>

                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-start gap-3">
                            <a href="{{ route('home') }}#contact"  class="inline-flex items-center justify-center bg-[#4f6ba3] hover:bg-[#3e5b8f] text-white font-semibold py-3 px-6 rounded-full shadow-lg transition-transform transform hover:-translate-y-1">{{ __('services.buttons.contact') }}</a>
                            <a href="{{ route('home') }}#contact"  class="inline-flex items-center justify-center border border-gray-200 dark:border-gray-600 px-5 py-3 rounded-full text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition">{{ __('services.buttons.request_service') }}</a>
                        </div>
                    </div>
                </div>
            </div>
